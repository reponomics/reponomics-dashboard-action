...
Use `encrypted` as the default. It protects retained artifacts and hosted dashboard data from people who do not have the dashboard key. It does not hide:
...
Encrypted mode is still a shared-secret model, not per-user authentication. The action requires the secret to be non-empty but does not enforce entropy. See [Security Info](security-info.md) for key-generation and threat-model guidance.

## Plaintext

Use `plaintext` only in private repositories where GitHub repository and artifact access are the intended privacy boundary. It uploads retained CSV files directly inside the `dashboard-data` artifact, may upload a downloadable plaintext dashboard artifact, and does not publish a hosted Pages dashboard. The plaintext HTML artifact uses the same summary plus per-repository chunk data model as encrypted dashboards for runtime memory behavior, but without encryption.

The action rejects `plaintext` in public repositories.
...
For plaintext mode, users inspect the retained CSV files by downloading the `dashboard-data` workflow artifact.

...
- `publish`: publishing the data dashboards is done in a separate mode, called `publish`. `publish` is responsible for taking the raw data and producing the visual assets like the HTML dashboard and the README dashboard, and then serving the dashboard to GitHub Pages, when enabled.

- `rotate-key`: the Reponomics Dashboard makes it easy for users to rotate their key in case they lose access to it. So long as the previous key is still stored in the repo as a repository secret, the workflow is able to download the encrypted artifact and decrypt it. So, all that's needed for key rotation is a new key. This should give you peace of mind that losing your encryption key is not going to lead to permanent loss of your data. On the other hand, this means you must be extra vigilent about who is able to execute your workflows. _Anyone with the ability to set a new secret and run the `rotate-key` workflow is able to change your secret._

- `incident-reset`: this is designed to handle any scenario where your encryption key has been exposed publicly, exflitrated, or otherwise compromised, and you deem it ineessary to not only rotate to a new key, but destory all previous artifacts from previous workflow runs that have not expired. Workflow artifacts are acessible to anyone with read access to your repository - in the case of public repos, that means any GitHub user at all. Fortunately, it is possible to mitigate the damage - artifacts are not part of git history, so workflow runs and their artifacts can be deleted without any knock-on effects.

...
- [`template-release.yml`](template-release.yml) publishes generated template releases from merged template acceptance or template-only release PRs. It runs after relevant template-source changes land on `main`, skips when the matching generated-repository release already exists, runs `make template-release-gates`, uploads and attests deterministic template release artifacts, creates an immutable source tag, force-publishes the generated tree to `reponomics-dashboard`, and creates the public `reponomics-dashboard-v*` GitHub Release in the generated template repository. Before replacing generated `main`, it verifies or creates the previous generated-template archive tag from `.reponomics/template-provenance.json`.

- [`publish-template.yml`](publish-template.yml) is the emergency-only manual generated-template publication escape hatch. It requires explicit confirmation, runs the same release gates and artifact attestation path, then force-pushes the generated tree to `reponomics-dashboard`. It does not create the public generated-template GitHub Release and should not be used for routine releases.

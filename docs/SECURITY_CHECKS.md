...
The generated-template release artifact path follows the same rule. `template-release.yml` runs release gates and packages the generated template archive, canonical tree manifest, and `SHA256SUMS` as workflow artifacts before requesting protected `template-publication` deployment approval. After approval, it downloads the publication handoff, attests those files, creates the immutable source tag, mints the template-repository publication app token, and appends the generated publication commit. It does not depend on mutating release assets after publication; the public template release is created in the generated template repository after the generated commit and tag are verified.

...
- [`template-release.yml`](template-release.yml) creates source-repository `reponomics-dashboard-v*` template releases from merged template acceptance or template-only release PRs. It runs after relevant template-source changes land on `main`, skips when the template release already exists, runs `make template-release-gates`, and creates the matching GitHub Release from the merged commit.

- [`publish-template.yml`](publish-template.yml) builds the generated dashboard template from this repository and publishes it to `reponomics-dashboard`. It runs for `reponomics-dashboard-v*` template releases whose tag matches `template-contract.yml`, or for explicitly confirmed manual operator dispatches. For template releases, it runs `make template-release-gates`, which verifies the generated template, validates both the public compatible action ref and the accepted action release tag/SHA, runs generated-template checks against the accepted public action release, dry-runs publication, and packages deterministic template release artifacts. It then uploads the artifacts as workflow artifacts and creates GitHub artifact attestations before creating the template-repository app token and force-pushing the generated template; it does not mutate already-published immutable releases by uploading release assets after publication.

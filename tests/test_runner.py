...
def test_publish_surfaces_blocked_managed_docs_sync_status(
    ...
    monkeypatch.setenv("REPONOMICS_DOCS_SYNC_REASON", "Managed docs file differs from manifest: README.md")
    managed_docs_dir = tmp_path / "docs" / "reponomics"
    managed_docs_dir.mkdir(parents=True)
    (managed_docs_dir / "README.md").write_text("local docs\n", encoding="utf-8")
    config = _config(tmp_path, mode="publish", generate_readme=True)
    ...
    assert "Managed docs file differs from manifest: README.md" in readme
    assert "Local docs:" in dashboard
    assert "local edits block updates" in dashboard
    assert "Managed docs file differs from manifest: README.md" in dashboard
...
def test_publish_surfaces_stale_local_managed_docs_manifest(
    ...
    readme = config.readme_path.read_text(encoding="utf-8")
    dashboard = config.pages_index_path.read_text(encoding="utf-8")
    assert "**Local docs:** not current." in readme
    assert "Local managed docs were generated by action 0.12.0" in readme
    assert "Local docs:" in dashboard
    assert "not current" in dashboard
    assert "Local managed docs were generated by action 0.12.0" in dashboard

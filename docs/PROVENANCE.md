...
- Runtime Python dependencies are installed from the committed `requirements-runtime.lock` file in hash-required mode. The lock fixes the third-party Python package set used by the composite action, while CI checks that the lock remains current with `pyproject.toml`.
...
- The composite action installs runtime Python dependencies from `requirements-runtime.lock` with `pip --require-hashes` before executing `dashboard_action/run.py`. `make validate-runtime-lock` regenerates a temporary lock from `pyproject.toml`, compares it against the committed lock, and verifies that `pip` accepts the committed lock in hash-required mode.
